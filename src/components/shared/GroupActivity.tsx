...
//   return (
//     <div>
//       {convertedAmount ? `${amount} ${fromCurrency} = ${convertedAmount} ${toCurrency}` : 'Converting...'}
//     </div>
//   );
// };
...
//     } else {
//       amountMessage = `You get back $${parseFloat(activity.Amout).toFixed(2)}`;
//     }
...
//         </span>
//         <span className="text-blue-500 text-lg font-bold mt-2">
//         {activity.Currency} {activity.Amout}
//         </span>
//         <CurrencyConverter 
//           amount={parseFloat(activity.Amout)}
//           fromCurrency={activity.Currency}
//           toCurrency="USD" // You can make this dynamic based on user preference
//         />
//       </div>
...
//   // Add or remove 'active-modal' class to body based on modal state
//   useEffect(() => {
//     if (modal) {
//       document.body.classList.add("active-modal");
//     } else {
...
//   // Determine user's involvement in the expense
//   const isPaidByCurrentUser = activity.PaidBy.$id === currentUser?.$id;
...
//   // Calculate and format the amount message
//   let amountMessage = "";
//   if (isPaidByCurrentUser) {
//     if (isCurrentUserInvolved) {
//       const individualAmount = parseFloat(activity.Amout) / splitCount;
//       const getback = parseFloat(activity.Amout) - individualAmount;
//       amountMessage = `You get back ${activity.Currency} ${getback.toFixed(2)}`;
//     } else {
//       amountMessage = `You get back ${activity.Currency} ${parseFloat(activity.Amout).toFixed(2)}`;
//     }
//   } else if (isCurrentUserInvolved) {
//     const individualAmount = parseFloat(activity.Amout) / splitCount;
//     amountMessage = `You owe ${activity.Currency} ${individualAmount.toFixed(2)}`;
//   } else {
//     amountMessage = `Not involved`;
//   }
...
//         </div>
//         <span className="text-blue-500 text-lg font-bold mt-2">
//           {activity.Currency} {parseFloat(activity.Amout).toFixed(2)}
//         </span>
//         <CurrencyConverter 
//           amount={parseFloat(activity.Amout)}
//           fromCurrency={activity.Currency}
...
const GroupActivity = ({ activity }: UserCardProps) => {
  ...
  if (isPaidByCurrentUser) {
    if (isCurrentUserInvolved) {
      ...
      amountMessage = `You get back ${activity.Currency} ${userAmount.toFixed(2)}`;
    } else {
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between" }} className="pb-2">
        ...
        <span className="text-blue-500 text-lg font-bold mt-2">
          {activity.Currency} {totalAmount.toFixed(2)}
        </span>
      </div>
      ...
      <p className="text-gray-500">
        Individual split: {activity.Currency} {individualAmount.toFixed(2)}
      </p>
      <CurrencyConverter 
        amount={totalAmount}
        fromCurrency={activity.Currency}
