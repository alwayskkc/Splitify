...
export async function getGroups() {
  try {
    ...
    if (!groups || groups.documents.length === 0) {
      console.error("No groups found or data is malformed");
      return []; // Return an empty array instead of undefined or null
    }
...
export async function getGroupsActivityById(groups: string[]) {
  try {
    if (!groups || !Array.isArray(groups) || groups.length === 0) {
      ...
      console.error("Groups array is undefined, not an array, or empty.");
    ...
    const groupActivities = await Promise.all(
      groups.map(async (group: any) => {
        ...
        if (groupData && groupData.activity) {
